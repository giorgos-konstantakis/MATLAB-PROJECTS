...
% M_PSK's order
M=8; % choose  M=4(one symbol per 2 bits) or M=8(one symbol per 3 bits) or M=16(1 symbol per 4 bits)
...
[output_sequence_of_bits,input_sequence_of_bits]=M_PSK(Input,M,Lb,Tc,Tsample,Tsymbol,SNR,Es,coding);

% you can check the error of bits transmmited with biterr
